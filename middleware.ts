...
import {validDashboardUser,checkPassword,digest,gatePage,hex,readSession,sameOrigin,sessionHeader,sessionSeconds} from './lib/dashboard-access';
...
export async function middleware(request:Request){
 ...
 try{
  ...
  if(path==='/access/unlock'&&request.method==='POST'){
   ...
   const body=await request.text();if(body.length>2048)return page('Password input is too long.',400);const fields=new URLSearchParams(body),password=fields.get('password')||'',userId=fields.get('userId');
   if(!validDashboardUser(userId))return page('Select Addison, Heather, or Makayla.',400);
   if(!password||password.length>128||!await checkPassword(password,hash))return page('Incorrect password. Try again.',401);
   const token=hex(crypto.getRandomValues(new Uint8Array(32)));await db.prepare('INSERT INTO dashboard_sessions(tokenHash,expires,userId) VALUES (?,?,?)').bind(await digest(token+hash),now+sessionSeconds,userId).run();
  ...
  const session=key?await db.prepare('SELECT expires,userId FROM dashboard_sessions WHERE tokenHash=? AND expires>?').bind(key,now).first():null;
  if(path==='/access')return page();
  if(!session||!validDashboardUser(session.userId)){if(path.startsWith('/api/')||request.method!=='GET'&&request.method!=='HEAD')return Response.json({error:'Unlock the dashboard to continue.',authenticationRequired:true},{status:401,headers:noStore});return page()}
  if(path==='/access/me')return Response.json({userId:session.userId},{headers:noStore});
