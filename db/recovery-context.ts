...
export function withWriteEpoch(handler:(req:Request)=>Promise<Response>){return async(req:Request)=>{try{const epoch=await currentEpoch(),value=req.headers.get(epochHeader);if(value===null?epoch!==0:!/^\d+$/.test(value)||Number(value)!==epoch)return stale();const response=await writeEpoch.run(epoch,()=>handler(req));if(!response.ok&&await currentEpoch()!==epoch)return stale();return response;}catch{return Response.json({error:'The save could not be verified. No confirmation was returned.'},{status:503})}}}
