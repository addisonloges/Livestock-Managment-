...
async function handleGET(){try{
 ...
 const bytes=new TextEncoder().encode(JSON.stringify({format:'clarksons-full-backup',identityCounters:(await db.prepare('SELECT * FROM identity_counters').all()).results,recoveryHistory:(await db.prepare('SELECT * FROM recovery_runs ORDER BY createdAt').all()).results,version:1,exportedAt:new Date().toISOString(),includesMediaBytes:true,files,tables:data}));
