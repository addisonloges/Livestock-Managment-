...
export default function DashboardLock(){
 ...
 useEffect(()=>{fetch('/access/me').then(r=>r.json()).then((d:any)=>setUser(d.userId||'')).catch(()=>{})},[]);
 ...
 return <><span className="text-sm font-semibold">{user}</span><Button variant="outline" size="sm" disabled={busy} onClick={lock}>{busy?'Locking…':'Lock dashboard'}</Button>{error&&<span role="alert">{error}</span>}</>
}
