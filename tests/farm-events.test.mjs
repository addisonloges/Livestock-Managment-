import test from 'node:test';import assert from 'node:assert/strict';import {allocation,animalExpenses,validateEvent,animalIncome,cashSummary,lotBalance,rationCost,treatmentWarnings} from '../lib/farm-events.ts';
...
test('foster records stay separate from biological parentage and reject self-fostering',()=>{const animal={id:crypto.randomUUID(),species:'Sheep',sex:'Female',firstYear:2026,dob:'2026-01-01',dam:'biological'},dam={id:crypto.randomUUID(),species:'Sheep',sex:'Female',firstYear:2024,dob:'2024-01-01'};const e={id:crypto.randomUUID(),kind:'rearing',species:'Sheep',date:'2026-02-01',title:'Fostered',notes:'',category:'Fostered',dueDate:'',amountCents:null,animalIds:[animal.id],rearing:{fosterDamId:dam.id,numberReared:2}};validateEvent(e,[animal,dam]);assert.equal(animal.dam,'biological');assert.throws(()=>validateEvent({...e,rearing:{...e.rearing,fosterDamId:animal.id}},[animal,dam]));assert.throws(()=>validateEvent({...e,category:'Bottle-raised'},[animal,dam]));});

test('individual treatment states control inventory and repeat warnings include a protocol',()=>{const lot={id:'lot',kind:'stock',stock:{quantity:20,unit:'ml',minimumIntervalDays:7}},e={id:'e',kind:'treatment',animalIds:['a','b'],protocol:{lotId:'lot',product:'QA',doses:[{date:'2026-01-01',amount:2,unit:'ml',state:'Given',states:{b:'Skipped'}},{date:'2026-01-03',amount:2,unit:'ml',state:'Scheduled',states:{b:'Given'}}]}};assert.equal(lotBalance(lot,[e]).used,4);assert.ok(treatmentWarnings(e,[lot]).some(w=>w.includes('minimum review interval')));assert.equal(lotBalance(lot,[{...e,voided:true}]).used,0);});
