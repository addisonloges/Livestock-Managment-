...
assert.throws(()=>tagChange(a,{field:'rightTag',mode:'retire',value:'2',date:'2026-01-01'},2026),/color/);
...
let row={...blankImportRow('Sheep','2026','1'),name:'Color QA',rightTag:'42'};assert.ok(prepareImport([row],[],2026).errors.some(e=>e.includes('color')));row.rightTagColor='Yellow';assert.equal(prepareImport([row],[],2026).errors.length,0);row.rightTag='   ';row.rightTagColor='';assert.equal(prepareImport([row],[],2026).errors.length,0);
