...
Pedigree-update imports recognize Sire Reg, Sire Flock Name/ID, Dam Reg and Dam Flock Name/ID. They match existing animals by tags, EID, registration, or name and birth date; explicit match selection handles renamed animals. Shared parents are reused by registration or exact flock identity and created as unowned ancestor records when needed. Existing conflicting links stop the update, unmatched rows are explicitly listed, and a transaction saves links, registration metadata and change history together. Repeating a completed update does not create duplicate parents.
