import {eventAllocation,type FarmEvent} from './farm-events.ts';
export function retainedEstimate(events:FarmEvent[],animalId:string,through:string){
 ...
 return {cents:eventAllocation(latest).find(a=>a.id===animalId)!.cents,date:latest.date,conflict:false};
}
